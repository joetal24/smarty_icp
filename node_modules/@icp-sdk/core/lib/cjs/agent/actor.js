...
class Actor {
    ...
    static canisterIdOf(actor) {
        return _principal_1.Principal.from(actor[metadataSymbol].config.canisterId);
    }
    static createActorClass(interfaceFactory, options) {
        const service = interfaceFactory({ IDL: _candid_1.IDL });
        class CanisterActor extends Actor {
            constructor(config) {
                ...
                const canisterId = typeof config.canisterId === 'string'
                    ? _principal_1.Principal.fromText(config.canisterId)
                    : config.canisterId;
                super({
                    config: {
...
function decodeReturnValue(types, msg) {
    const returnValues = _candid_1.IDL.decode(types, msg);
...
function _createActorMethod(actor, methodName, func, blsVerify) {
    ...
    if (isQueryAnnotated && queryStrategy === 'query') {
        caller = async (options, ...args) => {
            ...
            const cid = _principal_1.Principal.from(options.canisterId || actor[metadataSymbol].config.canisterId);
            const arg = _candid_1.IDL.encode(func.argTypes, args);
        ...
        caller = async (options, ...args) => {
            ...
            const cid = _principal_1.Principal.from(canisterId);
            const ecid = effectiveCanisterId !== undefined ? _principal_1.Principal.from(effectiveCanisterId) : cid;
            const arg = _candid_1.IDL.encode(func.argTypes, args);
            let reply;
            let certificate;
            let callResponse;
            ...
            try {
                ({ reply, certificate, callResponse, requestDetails } = await agent.update(cid, {
                    methodName,
                    arg,
                ...
                if (e instanceof errors_ts_1.RejectError) {
                    ...
                    throw errors_ts_1.RejectError.fromCode(enrichedCode);
                }
                throw e;
            }
