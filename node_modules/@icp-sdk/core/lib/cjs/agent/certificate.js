...
exports.lookupCanisterRanges = lookupCanisterRanges;
exports.lookupCanisterRangesFallback = lookupCanisterRangesFallback;
exports.decodeCanisterRanges = decodeCanisterRanges;
exports.getSubnetIdFromCertificate = getSubnetIdFromCertificate;
const cbor = __importStar(require("./cbor.js"));
const errors_ts_1 = require("./errors.js");
const _principal_1 = require("../principal/index.js");
const _candid_1 = require("../candid/index.js");
...
class Certificate {
    ...
    static createUnverified(options) {
        return new Certificate(options.certificate, options.rootKey, options.principal, options.blsVerify ?? bls.blsVerify, options.maxAgeInMinutes, options.disableTimeVerification, options.agent);
    }
    ...
    async verify() {
        ...
        const msg = (0, utils_1.concatBytes)(constants_ts_1.IC_STATE_ROOT_DOMAIN_SEPARATOR, rootHash);
    ...
    async _checkDelegationAndGetKey(d) {
        ...
        if (isCanisterPrincipal(this._principal)) {
            ...
            subnetId = _principal_1.Principal.fromUint8Array(d.subnet_id);
        ...
        if (!publicKeyLookup) {
            if (isSubnetPrincipal(this._principal)) {
                throw errors_ts_1.TrustError.fromCode(new errors_ts_1.CertificateNotAuthorizedForSubnetErrorCode(subnetId));
            }
            else {
...
function lookupCanisterRanges(params) {
    ...
    if (shardPaths.length === 0) {
        throw errors_ts_1.ProtocolError.fromCode(new errors_ts_1.CertificateNotAuthorizedErrorCode(canisterId, subnetId));
    }
    shardPaths.sort(_candid_1.compare);
...
function lookupCanisterRangesFallback(subnetId, tree) {
    ...
    if (lookupResult.status !== LookupPathStatus.Found) {
        throw errors_ts_1.ProtocolError.fromCode(new errors_ts_1.LookupErrorCode(`Could not find canister ranges for subnet ${subnetId.toText()}`, lookupResult.status));
    }
...
function decodeCanisterRanges(lookupValue) {
    const ranges_arr = cbor.decode(lookupValue);
    const ranges = ranges_arr.map(v => [
        _principal_1.Principal.fromUint8Array(v[0]),
        _principal_1.Principal.fromUint8Array(v[1]),
    ]);
    return ranges;
}
function getCanisterRangeShardPaths(canisterRangeShards) {
    ...
    for (const path of list_paths([], canisterRangeShards)) {
        ...
        if (!firstLabel) {
            throw errors_ts_1.ProtocolError.fromCode(new errors_ts_1.CertificateVerificationErrorCode('Path is invalid'));
        }
...
function getCanisterRangeShardPartitionPoint(shardPaths, canisterId) {
    ...
    while (left < right) {
        ...
        if ((0, _candid_1.compare)(shardPaths[mid], canisterIdBytes) <= 0) {
...
function getCanisterRangeFromShards(maxShardPath, canisterRangeShards) {
    ...
    if (canisterRange.status !== LookupPathStatus.Found) {
        throw errors_ts_1.ProtocolError.fromCode(new errors_ts_1.LookupErrorCode(`Could not find canister range for shard ${maxShardPath.toString()}`, canisterRange.status));
    }
...
function getSubnetIdFromCertificate(certificate, rootKey) {
    if (certificate.delegation) {
        return _principal_1.Principal.fromUint8Array(certificate.delegation.subnet_id);
    }
    // If certificate has no delegation, it comes from the root subnet
    return _principal_1.Principal.selfAuthenticating(rootKey);
}
